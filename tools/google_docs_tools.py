...
class GoogleDocsManager:
    ...
    async def add_document(self, content, doc_title, folder_name, make_shareable=False, folder_shareable=False, markdown=False):
        """
        ...
        try:
            ...
            if markdown:
                ...
                doc_id = await self._convert_markdown_to_google_doc(content, doc_title)
            else:
    ...
    async def _convert_markdown_to_google_doc(self, markdown_content, title):
        """Convert Markdown content to a Google Document."""
        try:

            # Ensure markdown_content is awaited if it's a coroutine
            if asyncio.iscoroutine(markdown_content):
